<?php
//create src directory in root level and inside the src create another directory name foundationphp
namespace foundationphp;

class UploadFile{
	protected $destination;
	protected $messages = array();
	protected $maxSize = 51200; //50kb 50 * 1024
	...
	protected function moveFile($file){
		...
		$success = move_uploaded_file($file['timp_name'], $this->destination . $filename);
		if($success){
			$result =  $file['name'] . ' was uploaded successfully';
			if(!is_null($this->newName)){
				$result .= ', and was rename to ' . $this->newName;
			}
			$result .= '.';
